...
def train_ddqn_her(env, agent, replay, episodes=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.999, k_future=4, batch_size=128, start_scores=None):
    ...
    for ep in range(start_ep, total_episodes):
        ...
        if (ep + 1) % 10 == 0:
            ...
            plt.title('DDQN HER Training Scores')
            plt.xlabel('Episode')
            plt.ylabel('Score')
            plt.legend()
            plt.savefig(os.path.join(current_dir, "scores_ddqn_her.png"))
            plt.close()
...
if __name__ == "__main__":
    ...
    train_ddqn_her(env, agent, replay, episodes=1000, eps_start=1.0, start_scores=start_scores)
