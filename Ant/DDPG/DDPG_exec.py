...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden=(400, 300), act=nn.ReLU, layer_norm=True):
        super().__init__()
        layers = []
        last = in_dim
        for h in hidden:
...
class Actor(nn.Module):
    ...
    def forward(self, obs):
        return self.tanh(self.body(obs))  # [-1, 1] per action dim
...
class DDPGAgent:
    def __init__(
        ...
        self.actor = Actor(obs_dim, act_dim, actor_hidden).to(device) # actor
        self.actor_target = Actor(obs_dim, act_dim, actor_hidden).to(device) # actor의 느린 복사복
        self.critic = Critic(obs_dim, act_dim, critic_hidden).to(device) # critic
        self.critic_target = Critic(obs_dim, act_dim, critic_hidden).to(device) # critic_target의 복사본
    ...
    def act(self, obs: np.ndarray, noise: np.ndarray | None = None): #행동을 뽑는 단계
        ...
        return a_np.astype(np.float32)               # (act_dim,)
...
def train_ddpg(env, agent, replay, episodes=300, start_steps=10000, batch_size=128, noise_scale=0.3, render=False):
    returns = []
    ou_noise = OUNoise(size=env.action_space.shape[0])
    # total_Step = 0
    obs, _ = env.reset()
    for ep in range(episodes):
        ep_return = 0
        ou_noise.reset()
        for step in range(1000):  # Ant-v4 default max_episode_steps=1000
            # total_Step += 1
        ...
        df.to_excel("./Ant/DDPG/episode_rewards.xlsx", index=False)
    return returns
...
if __name__ == "__main__":
    ...
    agent = DDPGAgent(obs_dim=obs_dim, act_dim=act_dim, actor_hidden=(400,300), critic_hidden=(400,300))
    replay = ReplayBuffer(obs_dim, act_dim, size=200000)
    train_ddpg(env, agent, replay, episodes=1000, start_steps=25000, batch_size=128, noise_scale=0.2, render=False)

    # --- Save trained parameters ---
    torch.save(agent.actor.state_dict(), "./Ant/DDPG/actor.pth")
    torch.save(agent.critic.state_dict(), "./Ant/DDPG/critic.pth")
    print("모델 저장 완료: actor.pth , critic.pth")

    import pandas as pd
    import matplotlib.pyplot as plt

    df = pd.read_excel("./Ant/DDPG/episode_rewards.xlsx")
